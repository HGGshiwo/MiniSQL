...
3.表信息提供的接口是：```catalog_manager.catalog_buffer```，这个是字典，直接赋值和读取即可

4.如果修改了```catalog_manager.catalog_buffer```，一定要写入，使用```self.write_index```
...
| 类名            | 解释                                                         |
| --------------- | ------------------------------------------------------------ |
| buffer_manager  | 基类，提供了文件读写，如果需要读写文件的类必须继承它         |
