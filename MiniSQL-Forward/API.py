...
class Api(IndexManager):
    ...
    def create_index(self, table_name, index_name):
        ...
        if self.catalog_list.count(table_name) != 0 : 
            ...
            while page_no != -1:
                ...
                #转到下一个page
            #升序排列
            res.sort(key=self.secondelement)
            #print(res)
            # 为该表开辟文件空间存储
            fmt = ''
            fmt = fmt + struct.unpack_from(str(fmt_size) + 's', self.pool.buf, (addr << 12) + Off.fmt)[primary_key_location]
    ...
    def drop_table(self,table_name):
        if self.catalog_list.count(table_name) != 0 : 
            ...
            catalog_num = (len(table) - 2) // 4
            for i in range(0, catalog_num):  
                if(table[(i << 2) + 2 + 3] != -1 & i != primary_key_location) : #若存在索引，不为主键
                    self.delete_index(table_name,table[(i << 2) + 2 ])
